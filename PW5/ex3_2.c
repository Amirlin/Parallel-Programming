...
int main(int argc, char **argv)
{
    ...
    if (rank == 0)
    {
        ...
        code = MPI_Recv(my_array, N, MPI_FLOAT, 1, tag, MPI_COMM_WORLD, MPI_STATUSES_IGNORE);
        printf("array recieved back by process 0\n");
    }
    else if (rank == 1)
    {
        ...
        code = MPI_Send(recieve, N, MPI_FLOAT, 0, tag, MPI_COMM_WORLD);
    }
